...
from bankroll.broker import AccountData, parsetools, configuration
...
def _parseOption(symbol: str) -> Option:
    ...
    return Option(
        ...
        expiration=date(int(match["year"]), int(match["month"]), int(match["day"])),
...
def _parseSchwabPosition(p: _SchwabPosition) -> Optional[Position]:
    ...
    return Position(
        ...
        costBasis=Cash(currency=Currency.USD, quantity=_schwabDecimal(p.costBasis)),
    )
...
def _schwabPositionsFromRows(rows: Iterable[List[str]]) -> Iterable[_SchwabPosition]:
    return (
...
def _parseCashValue(p: _SchwabPosition) -> Tuple[str, Cash]:
    return (
        p.symbol,
        Cash(currency=Currency.USD, quantity=_schwabDecimal(p.marketValue)),
    )
...
def _parseTransactions(path: Path, lenient: bool = False) -> List[Activity]:
    with open(path, newline="") as csvfile:
        ...
        rows = [
            _SchwabTransaction._make(r[0:-1])
            for r in reader
        ...
        rowsByDate = {
            d: list(t)
            for d, t in groupby(
                sorted(rows, key=lambda t: t.date), key=lambda t: t.date
            )
        }
...
def _fixUpShortSales(
    activity: Sequence[Activity], inboundTransfers: Sequence[Trade]
) -> List[Activity]:
    ...
    def f(t: Activity) -> Activity:
        ...
        if (
            pos < 0
            and t.quantity > 0
            and t.quantity <= abs(pos)
            and t.flags & TradeFlags.OPEN
        ):
            return dataclasses.replace(
                t, flags=(t.flags ^ TradeFlags.OPEN) | TradeFlags.CLOSE
            )
        ...
        # If we find a short sale, see if there's a later transfer, and if so, record as closing a position.
            ...
            txs = (
                tx
                for tx in reversed(inboundTransfers)
                if tx.date >= t.date
                and tx.instrument.symbol == t.instrument.symbol
                and tx.quantity == abs(t.quantity)
            )
